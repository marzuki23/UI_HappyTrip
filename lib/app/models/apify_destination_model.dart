class ApifyDestination {
  ...
  factory ApifyDestination.fromJson(Map<String, dynamic> json) {
    ...
    String rawAddress = json['subTitle'] ?? json['address'] ?? 'Lokasi Tidak Diketahui';
    ...
    if (rawAddress != 'Lokasi Tidak Diketahui') {
      ...
      if (parts.length >= 2) {
        ...
        if (targetPart.toLowerCase().contains('jawa') || targetPart.toLowerCase().contains('indonesia')) {
    ...
    return ApifyDestination(
      ...
      city: cleanedCity, // Hasil kota yang sudah bersih dari alamat panjang & koordinat
